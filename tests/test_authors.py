...
def tests_get_authors_no_records(client):
    response=client.get('/api/v1/authors')#bez tokena
    expected_result= {
     'success': True,
     'data': [],
     'number_of_records': 0,
...
def test_get_authors_with_params(client, sample_data):
    ...
    #patrząc do naszej bazy powinny zostać zwrócone następujące rekordy:
...
def test_get_single_author(client, sample_data):
    ...
    assert response.headers['Content-Type']=='application/json'
    assert response_data['success'] is True
    #rekord o numerze 9 to andrzej sapkowski, ma 1 ksiazke
...
def test_create_author(client,token,author):
    response = client.post('/api/v1/authors',json=author,headers={
